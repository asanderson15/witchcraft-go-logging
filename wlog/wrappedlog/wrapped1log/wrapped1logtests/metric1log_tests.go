...
func metric1LogJSONOutputTests(t *testing.T, entityName, entityVersion string, loggerProvider func(w io.Writer) metric1log.Logger) {
	for i, tc := range Metric1TestCases(entityName, entityVersion) {
...
func extraValuesDoNotAppear(t *testing.T, entityName, entityVersion string, loggerProvider func(w io.Writer) metric1log.Logger) {
...
func tagIsntOverwrittenByTags(t *testing.T, entityName, entityVersion string, loggerProvider func(w io.Writer) metric1log.Logger) {
...
// Verifies that parameters remain separate between different logger calls (ensures there is not a bug where parameters
// are modified by making a logger call).
func extraTagsDoNotAppear(t *testing.T, entityName, entityVersion string, loggerProvider func(w io.Writer) metric1log.Logger) {
